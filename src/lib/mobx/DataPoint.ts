import {
  onBecomeObserved,
  onBecomeUnobserved,
  runInAction,
  reaction,
  makeObservable,
  observable,
  action,
  FlowCancellationError,
  computed,
} from 'mobx';

import { fetchData, getErrorMessage, isServer } from 'lib/utils';
import { CancellableOrPromise } from 'lib/types/common';

type DataPointParameters<T, D extends Array<unknown>> = {
  readonly getFetch: { (): ((...args: [...D, AbortSignal]) => CancellableOrPromise<T>) | null };
  readonly getDeps: { (): D | undefined | null };
  readonly initial?: T;
};

export const dataPoint = <T, D extends Array<unknown>>({
  getFetch,
  getDeps,
  initial,
}: DataPointParameters<T, D>) => {
  return new DataPoint<T, D>({ getFetch, getDeps, initial });
};

export class DataPoint<T, D extends Array<unknown>> {
  readonly data = fetchData<T>(this.params.initial);

  readonly map = <K>(f: (val: T) => K): DataPoint<K, D> => {
    // eslint-disable-next-line @typescript-eslint/no-this-alias
    const self = this;

    return makeObservable(
      {
        map: <U>(g: (val: K) => U) => {
          return self.map((val: T) => g(f(val)));
        },
        get data() {
          return {
            ...self.data,
            value: self.data.value && f(self.data.value),
          };
        },
      },
      {
        data: computed,
      },
    ) as DataPoint<K, D>;
  };

  constructor(private readonly params: DataPointParameters<T, D>) {
    makeObservable(this, {
      data: observable,
      __forceUpdate__: action,
    });

    if (!isServer) this.__runAutoFetcher__();
  }

  readonly __forceUpdate__ = async (...args: D) => {
    const fetch = this.params.getFetch();

    if (fetch) {
      const abortController = new AbortController();

      this.cancelFetch();

      this.data.status = 'Loading';

      try {
        const awaitedData = fetch(...args, abortController.signal);

        this.cancelFetch = () => {
          if ('cancel' in awaitedData) awaitedData.cancel();
          abortController.abort();
        };

        const data = await awaitedData;

        runInAction(() => {
          this.data.status = 'Succeed';
          this.data.value = data;
        });
      } catch (error) {
        if (error instanceof FlowCancellationError) return;

        runInAction(() => {
          this.data.status = 'Error';
          this.data.error = getErrorMessage(error);
        });
      }
    }
  };

  readonly __runAutoFetcher__ = () => {
    onBecomeObserved(this, 'data', () => {
      this.disposeListener = reaction(
        () => [this.params.getFetch(), this.params.getDeps()] as const,
        async ([fetch, args]) => {
          if (fetch && args) {
            const abortController = new AbortController();

            this.cancelFetch();

            this.data.status = 'Loading';

            try {
              const awaitedData = fetch(...args, abortController.signal);

              this.cancelFetch = () => {
                if ('cancel' in awaitedData) awaitedData.cancel();
                abortController.abort();
              };

              const data = await awaitedData;

              runInAction(() => {
                this.data.status = 'Succeed';
                this.data.value = data;
              });
            } catch (error) {
              if (error instanceof FlowCancellationError) return;

              runInAction(() => {
                this.data.status = 'Error';
                this.data.error = getErrorMessage(error);
              });
            }
          }
        },
        { fireImmediately: true },
      );
    });

    onBecomeUnobserved(this, 'data', () => {
      this.disposeListener();
      this.cancelFetch();
    });
  };

  private disposeListener: () => void = () => void 0;
  private cancelFetch: () => void = () => void 0;
}
